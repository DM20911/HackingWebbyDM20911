...
> **Copilote de Hacking Web Offensif** — reconnaissance, exploitation, corrélation assistée par IA, chaînage d'attaques et reporting senior multi-format.

🌐 **Langues :** [Español](README.md) · [English](README.en.md) · [Português](README.pt.md) · [Français](README.fr.md) · [Deutsch](README.de.md) · [Italiano](README.it.md) · [日本語](README.ja.md)

Skill Claude Code / Claude Desktop pour les professionnels du pentesting web offensif. Regroupe la méthodologie, l'outillage, les MCP recommandés et un générateur de rapports multi-format intégré (Markdown, Word, PDF, HTML, PowerPoint, JSON, SARIF).
...
Copilote offensif de bout en bout :

- **Recon / OSINT / ASM** — sous-domaines, endpoints, JS intel, source maps, recherche de secrets.
- **Fuzzing & Scanners** — ffuf, feroxbuster, nuclei, sqlmap, dalfox, XSStrike.
- **Exploitation** — SQLi, XSS, SSRF, SSTI, désérialisation, IDOR/BOLA, mass assignment.
- **APIs en profondeur** — REST, GraphQL (introspection, batching, depth, alias), SOAP.
- **Auth deep dive** — JWT, OAuth2/OIDC, SAML, sessions, bypass MFA, race conditions.
- **Cloud + K8s + CI/CD** — AWS/GCP/Azure SSRF + IAM, kube-hunter, abus de GitHub Actions.
- **Sécurité du navigateur** — bypass CSP, postMessage, Service Workers, XS-Leaks, SameSite.
- **Abus de logique métier** — workflows, race, coupon/refund/transfert, escalade.
- **Chaînage d'attaques** — corrélation intelligente pour construire des chaînes critiques.
- **Reporting senior** — CVSS 3.1 + 4.0, CWE, CAPEC, ATT&CK, attack chains, preuves ; export md/docx/pdf/html/pptx.
...
- `/hackweb`
- `/HackingWebbyDM20911`
- *« audite ce site »*, *« recon target.cl »*, *« pentest cette API »*
- *« exploite cette vuln »*, *« cherche BOLA / IDOR »*, *« SSRF dans ce paramètre »*
- *« vérifie OAuth/JWT »*, *« attaque ce GraphQL »*
- *« génère le rapport offensif »*

La skill demande toujours la méthodologie et les données minimales avant toute exécution active.

---

## Boot — sélection de méthodologie

Première étape, avant toute exécution active :

```
Avec quelle méthodologie souhaites-tu travailler ?
  1. PTES
  2. OWASP WSTG
  3. NIST SP 800-115
  4. OSSTMM
  5. MITRE ATT&CK
  6. CWE / CAPEC
  7. Hybride (combiner plusieurs)
  8. Sélection manuelle
```

Le choix est persisté en project memory pour ne plus demander dans les sessions suivantes. Voir `references/methodologies.md`.
...
flowchart TB
    ...
    G["🧠  Corrélation IA &amp; Raisonnement de Chaînes"]:::ai
    H["📦  PoC Reproductible + Preuves<br/>CVSS 3.1 · CVSS 4.0 · CWE · CAPEC · ATT&amp;CK"]:::poc
    I["📄  Reporting Multi-format<br/>md · txt · html · docx · pdf · pptx · json · sarif"]:::report

    A --> B --> C --> D --> E --> F --> G --> H --> I
    A -.->|continu| A
    G -.->|nouveaux vecteurs| E
```
...
# Extraction d'endpoints depuis JS
...
Script Python multi-format dans `scripts/generate_report.py`. Formats : `md`, `txt`, `html`, `docx`, `pdf`, `pptx`, `json`, `all`.

```bash
python3 scripts/generate_report.py --input findings.json --format docx --output rapport.docx
python3 scripts/generate_report.py --input findings.json --format all --output ./out/
```

Schéma d'entrée dans `assets/example_findings.json`. 
---

## 🤖 Support multi-IA (optionnel)

Bien que la skill soit née pour **Claude Code / Claude Desktop**, elle peut s'utiliser depuis d'autres copilotes sans toucher au cœur. Toute la logique d'host vit dans `adapters/`.

| Host | Fichier d'entrée |
|------|------------------|
| Claude Code / Desktop | `SKILL.md` natif |
| **Gemini CLI** | `adapters/gemini/GEMINI.md` |
| **Cursor** | `adapters/cursor/.cursorrules` |
| **Aider** | `adapters/aider/CONVENTIONS.md` |
| **OpenAI Codex CLI / générique** | `adapters/openai-codex/AGENTS.md` |

```bash
bash adapters/install.sh                       # détection auto
bash adapters/install.sh --host gemini         # forcer un host
```

Détails dans [`adapters/README.md`](./adapters/README.md).
...
Claude Desktop : packager avec `zip -r` et importer dans Settings → Capabilities → Skills.

---

## Philosophie

> Les outils ne sont pas l'essentiel. L'essentiel c'est de comprendre HTTP, la logique métier, les flows d'auth, les frontières de confiance, et de penser en chaînes d'attaque.

Cette skill ne remplace pas le pentester — elle le rend 10x plus rapide.
