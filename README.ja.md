...
> **オフェンシブ Web ハッキング コパイロット** — レコン、エクスプロイト、AI 支援相関分析、アタックチェイン構築、シニアレベルのマルチフォーマットレポーティング。

🌐 **言語：** [Español](README.md) · [English](README.en.md) · [Português](README.pt.md) · [Français](README.fr.md) · [Deutsch](README.de.md) · [Italiano](README.it.md) · [日本語](README.ja.md)
...
エンドツーエンドのオフェンシブ コパイロット：

- **レコン / OSINT / ASM** — サブドメイン、エンドポイント、JS インテル、ソースマップ、シークレット ハンティング。
- **ファジング & スキャナ** — ffuf、feroxbuster、nuclei、sqlmap、dalfox、XSStrike。
- **エクスプロイト** — SQLi、XSS、SSRF、SSTI、デシリアライゼーション、IDOR/BOLA、Mass Assignment。
- **API 詳細テスト** — REST、GraphQL（イントロスペクション、バッチング、深度、エイリアス）、SOAP。
- **認証ディープダイブ** — JWT、OAuth2/OIDC、SAML、セッション、MFA バイパス、レース コンディション。
- **クラウド + K8s + CI/CD** — AWS/GCP/Azure SSRF + IAM、kube-hunter、GitHub Actions 悪用。
- **ブラウザ セキュリティ** — CSP バイパス、postMessage、Service Workers、XS-Leaks、SameSite。
- **ビジネス ロジック悪用** — ワークフロー、レース、クーポン/返金/送金、権限昇格。
- **アタックチェイン** — 重大なチェーンを構築するためのインテリジェントな相関分析。
- **シニア レベルのレポーティング** — CVSS 3.1 + 4.0、CWE、CAPEC、ATT&CK、アタックチェイン、エビデンス；md/docx/pdf/html/pptx へのエクスポート。

---

## 呼び出し方

**Claude Code** または **Claude Desktop** で：

- `/hackweb`
- `/HackingWebbyDM20911`
- *「この Web をオーディットして」*、*「target.cl にレコン」*、*「この API をペンテスト」*
- *「この脆弱性をエクスプロイト」*、*「BOLA / IDOR を探して」*、*「このパラメータで SSRF」*
- *「OAuth/JWT をレビュー」*、*「この GraphQL を攻撃」*
- *「オフェンシブ レポートを生成」*

スキルはアクティブなアクションを実行する前に、必ず方法論と最小限のデータを尋ねます。
...
選択はプロジェクト メモリとして保存され、同じエンゲージメントの後続セッションでは再度尋ねられません。`references/methodologies.md` を参照。
...
flowchart TB
    ...
    G["🧠  AI 相関分析 &amp; アタックチェイン推論"]:::ai
    H["📦  再現可能な PoC + エビデンス<br/>CVSS 3.1 · CVSS 4.0 · CWE · CAPEC · ATT&amp;CK"]:::poc
    I["📄  マルチフォーマット レポーティング<br/>md · txt · html · docx · pdf · pptx · json · sarif"]:::report
...
`scripts/generate_report.py` のマルチフォーマット Python スクリプト。フォーマット：`md`、`txt`、`html`、`docx`、`pdf`、`pptx`、`json`、`all`。
...
入力スキーマは `assets/example_findings.json` を参照。
---

## 🤖 マルチ AI サポート（オプション）

このスキルは **Claude Code / Claude Desktop** 向けに生まれましたが、コアを変更せずに他のコパイロットでも利用できます。ホスト固有のロジックはすべて `adapters/` に分離されています。

| ホスト | エントリ ファイル |
|--------|-------------------|
| Claude Code / Desktop | ネイティブ `SKILL.md` |
| **Gemini CLI** | `adapters/gemini/GEMINI.md` |
| **Cursor** | `adapters/cursor/.cursorrules` |
| **Aider** | `adapters/aider/CONVENTIONS.md` |
| **OpenAI Codex CLI / 汎用** | `adapters/openai-codex/AGENTS.md` |

```bash
bash adapters/install.sh                       # 自動検出
bash adapters/install.sh --host gemini         # ホストを強制
```

詳細は [`adapters/README.md`](./adapters/README.md) を参照。
...
Claude Desktop：`zip -r` でパッケージ化し、Settings → Capabilities → Skills でインポート。

---

## 哲学

> ツールが最も重要なのではない。重要なのは HTTP、ビジネス ロジック、認証フロー、信頼境界を理解し、アタックチェインで考えることだ。

このスキルはペンテスタを置き換えるものではなく、10倍速くするものだ。
