...
> **Copiloto de Hacking Web Ofensivo** — recon, exploração, correlação assistida por IA, attack chaining e relatórios sêniores multi-formato.
...
Skill para Claude Code / Claude Desktop para profissionais de pentesting web ofensivo. Empacota metodologia, ferramentas, MCPs recomendados e um gerador de relatórios próprio (Markdown, Word, PDF, HTML, PowerPoint, JSON, SARIF).

---

## O que faz

Copiloto ofensivo end-to-end:

- **Recon / OSINT / ASM** — subdomínios, endpoints, JS intel, source maps, caça de segredos.
- **Fuzzing & Scanners** — ffuf, feroxbuster, nuclei, sqlmap, dalfox, XSStrike.
- **Exploração** — SQLi, XSS, SSRF, SSTI, deserialization, IDOR/BOLA, mass assignment.
- **APIs em profundidade** — REST, GraphQL (introspection, batching, depth, alias), SOAP.
- **Auth deep dive** — JWT, OAuth2/OIDC, SAML, sessões, bypass de MFA, race conditions.
- **Cloud + K8s + CI/CD** — AWS/GCP/Azure SSRF + IAM, kube-hunter, abuso de GitHub Actions.
- **Segurança no browser** — CSP bypass, postMessage, Service Workers, XS-Leaks, SameSite.
- **Abuso de lógica de negócio** — workflows, race, cupom/refund/transferência, escalada.
- **Attack chaining** — correlação inteligente para construir cadeias críticas.
- **Reporting sênior** — CVSS 3.1 + 4.0, CWE, CAPEC, ATT&CK, cadeias de ataque, evidência; exportável para md/docx/pdf/html/pptx.

---

## Como invocar

Em **Claude Code** ou **Claude Desktop**:

- `/hackweb`
- `/HackingWebbyDM20911`
- *"audita esta web"*, *"recon a target.cl"*, *"pentest desta API"*
- *"explora esta vuln"*, *"caça BOLA / IDOR"*, *"SSRF neste parâmetro"*
- *"revisa OAuth/JWT"*, *"ataca este GraphQL"*
- *"gera o relatório ofensivo"*

A skill sempre pergunta a metodologia e os dados mínimos antes de executar qualquer ação ativa.

---

## Boot — seleção de metodologia

Primeiro passo, antes de qualquer execução ativa:

```
Com qual metodologia você quer trabalhar?
  1. PTES
  2. OWASP WSTG
  3. NIST SP 800-115
  4. OSSTMM
  5. MITRE ATT&CK
  6. CWE / CAPEC
  7. Híbrida (combinar várias)
  8. Seleção manual
```

A escolha persiste como project memory para não perguntar de novo nas sessões seguintes do mesmo engagement. Ver `references/methodologies.md`.
...
flowchart TB
    ...
    F["💥  Exploração<br/>SQLi · XSS · SSRF · SSTI · IDOR · BOLA · Race"]:::exploit
    G["🧠  Correlação IA &amp; Raciocínio de Cadeias"]:::ai
    H["📦  PoC Reproduzível + Evidência<br/>CVSS 3.1 · CVSS 4.0 · CWE · CAPEC · ATT&amp;CK"]:::poc
...
Script Python multi-formato em `scripts/generate_report.py`. Formatos: `md`, `txt`, `html`, `docx`, `pdf`, `pptx`, `json`, `all`.

```bash
python3 scripts/generate_report.py --input findings.json --format docx --output relatorio.docx
python3 scripts/generate_report.py --input findings.json --format all --output ./out/
```

Schema de input em `assets/example_findings.json`. 
---

## 🤖 Suporte multi-IA (opcional)

Embora a skill tenha nascido para **Claude Code / Claude Desktop**, também roda em outros copilotos sem tocar o núcleo. Toda lógica específica do host vive em `adapters/`.

| Host | Arquivo de entrada |
|------|--------------------|
...
| **OpenAI Codex CLI / genérico** | `adapters/openai-codex/AGENTS.md` |

```bash
bash adapters/install.sh                       # detecta o host
bash adapters/install.sh --host gemini         # forçar host
```

Detalhes em [`adapters/README.md`](./adapters/README.md).
...
Claude Desktop: empacotar com `zip -r` e importar em Settings → Capabilities → Skills.

---

## Filosofia

> As ferramentas não são o mais importante. O importante é entender HTTP, lógica de negócio, fluxos de auth, fronteiras de confiança e pensar em attack chains.

Esta skill não substitui o pentester — torna-o 10x mais rápido.
